...
export default function ResumeCandidateModal({
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cRes, pRes] = await Promise.all([
        ...
        fetch(`/api/candidates/${candidateId}/parse-status`, {
      ...
      if (!cRes.ok) {
        const msg =
          typeof cBody?.message === "string" ? cBody.message : cBody?.error;
  ...
  const resumeUrl =
    candidate && typeof candidate.resumeUrl === "string"
      ? candidate.resumeUrl.trim()
      : "";
  const resumeFileName =
    candidate && typeof candidate.resumeFileName === "string"
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          j?.message || j?.error || `Download failed (${res.status})`,
        );
      }
  ...
  async function handleParse(options = { force: false }) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/candidates/${candidateId}/resume/parse${q}`,
        {
          method: "POST",
          credentials: "same-origin",
        },
  ...
  useEffect(() => {
    ...
    setApplyYears(
      typeof y === "number" && Number.isFinite(y) ? String(y) : "0",
    );
    setApplySummary(
      typeof r.experience?.summary === "string" ? r.experience.summary : "",
    );
    setApplyOk(null);
  ...
  async function handleApplyParsedToProfile() {
    ...
    try {
      ...
      const skills = applySkills
        ...
        .filter(Boolean);
      const res = await fetch(
        `/api/candidates/${candidateId}/resume/parse/apply`,
        {
          method: "POST",
          credentials: "same-origin",
      ...
      <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-base">
            {candidateName ? `resume — ${candidateName}` : "resume"}
          </DialogTitle>
        ...
        {!candidateId ? (
          <p className="text-xs text-muted-foreground">
            No candidate ID for this row (local-only entry). resume actions
            require a saved application from the server.
          </p>
        ) : (
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-3">
              <div className="text-xs text-foreground/90">
                ...
                {resumeUrl ? (
                  <span className="font-mono bg-muted py-0.5 px-1.5 rounded">
                ...
                {canUpload && resumeUrl && (
                  <>
                    <Button
                      size="sm"
                      disabled={busy}
                      onClick={() => void handleParse({ force: false })}
                      className="h-8 text-xs font-semibold"
                    >
                      <ArrowsClockwise
                        className={cn("size-3.5 mr-1", busy && "animate-spin")}
                      />
                      Run parse job
                    </Button>
                    <Button
                      ...
                      size="sm"
                      disabled={busy}
                      onClick={() => void handleParse({ force: true })}
                      className="h-8 text-xs text-muted-foreground hover:text-foreground"
                      title="Enqueue a new job even if parsed before"
                    >
                      Force Re-parse
                    </Button>
                  </>
            ...
            <div className="p-4 border rounded-lg bg-muted/20 space-y-3">
              ...
              parseStatus?.status === "PROCESSING" ? (
                <p className="text-[10px] text-muted-foreground leading-normal">
                  Jobs stay pending until the parse worker runs (cron interval,
                  or manually hit `api/cron/process-parse-jobs`).
                </p>
              ) : null}

              {parseStatus?.error && (
                <Alert variant="destructive" className="py-2.5">
                  <AlertDescription className="text-[10px] font-mono whitespace-pre-wrap">
                    {parseStatus.error}
                  </AlertDescription>
                </Alert>
              )}
                  ...
                  <div className="border-t pt-4 space-y-4">
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground leading-normal mt-0.5">
                        Confirming will save the name, experience (years), and
                        parsed skills to the candidate database profile.
                      </p>
                    </div>

                    <div className="space-y-3">
                      <div className="flex flex-col gap-1.5">
                        <Label htmlFor="apply-name" className="text-xs">
                          Candidate Name
                        </Label>
                        <Input
                          id="apply-name"
                      ...
                      <div className="flex flex-col gap-1.5">
                        <Label htmlFor="apply-years" className="text-xs">
                          Experience (years)
                        </Label>
                        <Input
                          id="apply-years"
                      ...
                      <div className="flex flex-col gap-1.5">
                        <Label htmlFor="apply-skills" className="text-xs">
                      ...
                      <div className="flex flex-col gap-1.5">
                        <Label htmlFor="apply-summary" className="text-xs">
                          Experience Summary
                        </Label>
                        <Textarea
                          id="apply-summary"
            ...
            {parseStatus?.status === "COMPLETED" &&
              (applyOk || recommendationsRefreshKey > 0) && (
