  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center", marginBottom: 8 }}>
        ...
        {recMeta ? (
          <Badge label={recMeta.label} color={recMeta.color} bg={recMeta.bg} />
        ) : (
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {(canReschedule || canCancel || canFeedback || row.myFeedbackSubmitted) && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
            {canReschedule ? (
              <Button sm variant="ghost" onClick={() => onReschedule(row)}>
                Reschedule
              </Button>
            ) : null}
            {canCancel ? (
              <Button sm variant="danger" onClick={() => onCancel(row)}>
                Cancel
              </Button>
            ) : null}
            {canFeedback ? (
              <Button sm onClick={() => onFeedback(row)}>
                Submit feedback
              </Button>
            ) : null}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: compact ? 8 : 14 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
          {applicationId && canAct ? (
            <Button sm disabled={loading || busy || scheduleUsersLoading} onClick={() => void openScheduleModal()}>
      ...
      <Modal
        ...
        onClose={() => setRescheduleModal({ open: false, row: null })}
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <Field label="Duration (minutes)">
            <Input
              ...
              onChange={(e) =>
                setRescheduleForm((f) => ({ ...f, durationMinutes: e.target.value }))
              }
            />
          </Field>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Button sm variant="ghost" disabled={busy} onClick={() => setRescheduleModal({ open: false, row: null })}>
              Close
            </Button>
            <Button sm disabled={busy} onClick={() => void submitReschedule()}>
              {busy ? "Saving…" : "Save"}
            </Button>
          </div>
        </div>
      ...
      <Modal
        ...
        onClose={() => setCancelModal({ open: false, row: null })}
      ...
      >
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Button sm variant="ghost" disabled={busy} onClick={() => setCancelModal({ open: false, row: null })}>
              Close
            </Button>
            <Button sm variant="danger" disabled={busy} onClick={() => void submitCancel()}>
      ...
      <Modal
        open={scheduleModal}
        onClose={() => {
          if (!busy) setScheduleModal(false);
        }}
      ...
      >
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {scheduleUsersLoading ? (
            <p style={{ ...T.mono, fontSize: 12, color: "var(--text-muted)" }}>Loading interviewers…</p>
          ) : null}
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Button sm variant="ghost" disabled={busy} onClick={() => setScheduleModal(false)}>
              Close
            </Button>
            <Button sm disabled={busy || scheduleUsersLoading || !scheduleForm.interviewerUserId} onClick={() => void submitSchedule()}>
      ...
      <Modal
        ...
        onClose={() => setFeedbackModal({ open: false, row: null })}
      ...
      >
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Button sm variant="ghost" disabled={busy} onClick={() => setFeedbackModal({ open: false, row: null })}>
              Close
            </Button>
            <Button sm disabled={busy} onClick={() => void submitFeedback()}>
